...
## 2024-03-31 - [Avoid O(N²) loop mapping and sequential writes during state update]\n**Learning:** Found an $O(N²)$ mapping loop containing a `.find()` on state arrays and sequential `await dbHelper.put()` operations inside the topic renaming logic in `src/App.tsx`. This blocked the React thread during heavy updates.\n**Action:** When updating node state in `src/App.tsx` (or similar global state), avoid $O(N^2)$ nested array operations like `.find()` inside `.map()`, and sequential `await` IndexedDB writes. Combine filtering and mapping into a single $O(N)$ pass and batch database updates using `Promise.all()` to prevent blocking the React UI thread.
