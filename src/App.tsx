...
const KnowledgeGraph: FC<KnowledgeGraphProps> = memo(({ nodes, disciplinesMap, onNodeClick }) => {
  ...
  useEffect(() => {
    ...
    simulation.on("tick", () => {
      ...
      // ⚡ Bolt: Throttle expensive geometric background calculations (O(N log N) Convex Hull / Bezier)
      // to run only once every 3 ticks. This drastically reduces main thread blocking
      // while keeping the high-framerate physics simulation smooth for nodes/links.
      if (tickCount % 3 === 0) {
        // 🔄 动态更新所有学科的贝塞尔曲线多边形
        Array.from(disciplines).forEach(discipline => {
