...
## 2025-04-04 - Throttled D3 Geometric Calculations
**Learning:** Heavy geometric calculations in D3 visualizations (e.g. `generateBezierPath`, convex hull) can bottleneck the main thread if executed on every simulation `tick` event synchronously.
**Action:** Throttling such calculations to every N ticks (e.g., `tickCount % 3 === 0`) effectively mitigates the blocking without noticeably degrading the visual smoothness.
