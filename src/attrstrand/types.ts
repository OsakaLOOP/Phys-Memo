...
export interface EditionSubmission {
    conceptId: hash; // null if creating a new concept (will be generated by backend, or we provide it? Prompt says conceptId is calculated from name etc)
    // Actually if concept is new, conceptId shouldn't be here, but we can pass the name and topic instead.
    // Let's allow conceptId to be a temporary string if new, and the backend handles it. But we'll refine this in `core.ts`.

    // Actually, to make it clean, we should pass concept metadata if we are updating it.
    conceptName: string;
    conceptTopic: string;
    conceptDisciplines: string[];
